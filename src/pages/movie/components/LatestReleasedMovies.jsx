...
export default function LatestReleasedMovies({ title = "최신 개봉 영화" }) {
  ...
  useEffect(() => {
    const getLatestReleasedData = async () => {
      try {
        ...
        // 최신 개봉 영화 요청
        const response = await getLatestReleasedMovies();
        ...
        // 장르 ID를 장르 이름으로 변환
        const list = await addGenreNames(filteredContents, "movie");
